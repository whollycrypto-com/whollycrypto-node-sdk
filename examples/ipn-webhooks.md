...
Use [webhook-handler.mjs](webhook-handler.mjs) for either channel. Both send the same JSON; configure each receiver with the correct secret.
...
Mount `createWebhookHandler({projectId, signingSecret, enqueue})` after `express.raw({type: 'application/json', limit: '256kb'})`, before any JSON parser. Node.js 22+; Express is optional and not an SDK dependency. Supply your own transactional, persistent `enqueue` implementation. A failed commit must reject; an in-memory map is not a production inbox. For npm-free use, change the import to the extracted SDK's `dist/index.mjs` as shown in the main README.
...
```javascript
// After parseNotification() has verified the raw Buffer:
const status = notice.status;
const amountState = notice.payload.amount_status;
const invoiceTotal = notice.payload.amount; // "49.9", not USDC received
// In your durable worker, using the project's configured API host and token:
const detail = await client.getInvoice(configuredProjectId, notice.invoiceId);
