...
Use [webhook-handler.mjs](webhook-handler.mjs) as a receiver and [notification.json](notification.json) as a synthetic version-2 example. These examples send no payments and contain no real credentials.
...
The following is pseudocode, not a drop-in receiver. Use the state-based worker with the supplied [webhook-handler.mjs](webhook-handler.mjs); use the event-based worker only with an inbox that preserves distinct signed event IDs.
...
```javascript
await client.listInvoicePayments(projectId, invoiceId, {limit: 25, offset: 0})
```
